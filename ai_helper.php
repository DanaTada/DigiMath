...
// --- AI ģenerē jaunu uzdevumu, līdzīgu esošajam ---
function genere_uzdevums_similar($current_task, $grade) {
    $prompt = "Izveido vienu jaunu matemātikas uzdevumu ${grade}. klases skolēnam. "
        . "Uzdevumam jābūt LĪDZĪGAM šim, bet ne identiskam: \"$current_task\"\n"
        . "Izmanto līdzīgus skaitļus un to pašu darbību veidu (saskaitīšana, atņemšana, reizināšana, dalīšana, vai vienādojums).\n"
        . "Atbildi TIKAI ar JSON, bez paskaidrojumiem un bez markdown. Formāts: "
        . '{"uzdevums":"piemēram, 5 + 3 =","atbilde":"8"}';
    ...
    // Fallback to simple local generation if AI fails
    return genere_lokali_pēc_grade($grade);
}

// --- Rezerves variants lokālai ģenerācijai pēc klases ---
function genere_lokali_pēc_grade($grade) {
    if ($grade <= 2) {
        $a = rand(1, 20);
        $b = rand(1, 20);
        $operation = rand(0, 1) ? '+' : '-';
        ...
        return ['uzdevums' => "$a $operation $b =", 'atbilde' => (string)$answer];
    } elseif ($grade <= 4) {
        $a = rand(1, 10);
        $b = rand(1, 10);
        $operation = rand(0, 1) ? '×' : '÷';
